...
class Workbook {
  ...
  copySheet(sheetName, copyName, binary = true) {
    ...
    self.workbook.findall("definedNames/definedName").forEach((element) => {
      if (
        ...
        var newDefinedName = etree.SubElement(
          self.workbook.find("definedNames"),
          "definedName",
          element.attrib
        );
    ...
    self.archive.file(
      relArcName,
      etree.tostring(self.loadSheetRels(sheet.filename).root)
    );
        ...
        if (cell.attrib.t === "s") {
          ...
          self.extractPlaceholders(string).forEach(function (placeholder) {
            ...
            if (
              ...
              (substitution instanceof Array || Array.isArray(substitution))
            ) {
              ...
              if (newCellsInserted !== 0 || substitution.length) {
                ...
                if (substitution[0][placeholder.key] instanceof Array || Array.isArray(substitution[0][placeholder.key])) {
                  appendCell = false;
                }
              }
      ...
      substitution.forEach(function (element, idx) {
        ...
        if (idx === 0) {
          ...
          if (isMergeCell) {
            ...
            for (
              let colnum = self.charToNum(originalMergeStart.col) + 1;
              colnum <= self.charToNum(originalMergeEnd.col);
              colnum++
            ) {
              const originalRow = self.sheet.root
                .find("sheetData")
                ._children.find((f) => f.attrib.r == originalMergeStart.row);
              let col = self.numToChar(colnum);
              let originalCell = originalRow._children.find((f) =>
                f.attrib.r.startsWith(col)
              );
  ...
  initRichData() {
    if (!this.richDataIsInit) {
      ...
      const _relsrichValueRelFileName =
        "xl/richData/_rels/richValueRel.xml.rels";
      const rdrichvalueFileName = "xl/richData/rdrichvalue.xml";
      const rdrichvaluestructureFileName =
      ...
      if (this.archive.file(_relsrichValueRelFileName)) {
        this._relsrichValueRel = etree
          .parse(this.archive.file(_relsrichValueRelFileName).asText())
          .getroot();
      }
      if (this.archive.file(rdrichvalueFileName)) {
        this.rdrichvalue = etree
          .parse(this.archive.file(rdrichvalueFileName).asText())
          .getroot();
      }
      if (this.archive.file(rdrichvaluestructureFileName)) {
        this.rdrichvaluestructure = etree
          .parse(this.archive.file(rdrichvaluestructureFileName).asText())
          .getroot();
      }
      if (this.archive.file(rdRichValueTypesFileName)) {
        this.rdRichValueTypes = etree
          .parse(this.archive.file(rdRichValueTypesFileName).asText())
          .getroot();
      }
      if (this.archive.file(richValueRelFileName)) {
        this.richValueRel = etree
          .parse(this.archive.file(richValueRelFileName).asText())
          .getroot();
      }
      if (this.archive.file(metadataFileName)) {
        this.metadata = etree
          .parse(this.archive.file(metadataFileName).asText())
          .getroot();
      }
  ...
  writeRichData() {
    if (this.richDataIsInit) {
      ...
      const metadataFileName = "xl/metadata.xml";
      this.archive.file(
        _relsrichValueRelFileName,
        etree.tostring(this._relsrichValueRel)
      );
      this.archive.file(rdrichvalueFileName, etree.tostring(this.rdrichvalue));
      this.archive.file(
        rdrichvaluestructureFileName,
        etree.tostring(this.rdrichvaluestructure)
      );
      this.archive.file(
        rdRichValueTypesFileName,
        etree.tostring(this.rdRichValueTypes)
      );
      this.archive.file(
        richValueRelFileName,
        etree.tostring(this.richValueRel)
      );
      this.archive.file(metadataFileName, etree.tostring(this.metadata));

      const wbrelsidMax = this.findMaxId(
        this.workbookRels,
        "Relationship",
        "Id",
        /rId(\d*)/
      ...
      if (
        !this.writeRichDataAlreadyExist(
          this.workbookRels,
          "Relationship",
          "Target",
  ...
  substituteImageInCell(cell, substitution) {
    ...
    this.archive.file(
      ...
      this.toArrayBuffer(substitution),
      { binary: true, base64: false }
    );
    const maxIdRichData = this.findMaxId(
      this._relsrichValueRel,
      "Relationship",
      "Id",
      /rId(\d*)/
    );
    ...
    const futureMetadataCount = this.metadata
      .find("futureMetadata")
      .get("count");
    this.metadata
      .find("futureMetadata")
      .set("count", parseInt(futureMetadataCount) + 1);
    ...
    this.metadata
      .find("valueMetadata")
      .set("count", parseInt(valueMetadataCount) + 1);
  ...
  substituteImage(cell, string, placeholder, substitution, drawing) {
    ...
    try {
      ...
      if (
        self.option &&
        self.option.handleImageError &&
        typeof self.option.handleImageError === "function"
      ) {
    ...
    workbook.findall("definedNames/definedName").forEach(function (name) {
      ...
      if (self.isRange(ref)) {
        ...
        if (self.option && self.option.pushDownPageBreakOnTableSubstitution) {
          if (
            self.sheet.name == name.text.split("!")[0].replace(/'/gi, "") &&
            namedEnd
          ) {
